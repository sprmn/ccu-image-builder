# Generate a CCU image

## Prerequisites

1. You'll need to have access to the github repositories https://github.com/MrAlfabet/DCU-central-control-app and https://github.com/sprmn/ccu-server.
2. Git must be installed and you must be authenticated locally so that git can clone the above repositories.
3. Docker must be installed and the docker daemon must be running.

## Generating

1. Run `./create-ccu-image.sh`
2. Drink some coffee and wait (might take up to an hour)
3. A zip file containing the `.img` is stored at `deploy/image_YYYY-MM-DD-TegnisCCU.zip`.
